...
public final class Crud_A76 {
    ...
    public Cursor EXISTEN_FILAS_PENDIENTES_DE_REGISTRAR(String DIA) {
        SQLiteDatabase db = BASEDATOS.getReadableDatabase();
        String sql = String.format("SELECT * FROM %s WHERE %s NOT IN (SELECT %s FROM %s WHERE substr(%s, 0, 11) = ? AND %s=?) AND %s=?", TABLAS.FILA, IT_Fila.I_FILA.ID, I_A76.ID_FILA, TABLAS.A76, I_A76.FECHA_INGRESO, I_A76.ESTADO, IT_Fila.I_FILA.ESTADO);
        String[] selectionArgs = {DIA, "A", "A"};
        return db.rawQuery(sql, selectionArgs);
    }
